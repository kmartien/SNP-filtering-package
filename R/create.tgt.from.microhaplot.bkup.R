...
### This is a kludge to remove genotypes where there are more than haplotypes that haven't already been filtered out
genos.to.drop <- filter(genos, rank > 2) %>% filter(to.remove == FALSE)
genos$to.remove[which(genos$id.loc %in% genos.to.drop$id.loc)] <- TRUE
...
save(tgt, file = paste0("results/", project, ".tgt.rda"))
