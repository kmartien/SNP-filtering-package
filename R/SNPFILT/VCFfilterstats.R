...
read.ind.stats <- function(dir, vcf) {
  ...
  # join stats
  if(dim(het)[1] == 0) {
    print(paste0(filename, " is empty."))
    break
  } else  final <- left_join(temp, het)
}
...
read.loc.stats <- function(dir, fname) {
  # read depth stats
  filename <- paste(fname, ".ldepth.mean", sep = "")
  path <- file.path(dir, filename)
  ldepth <- read.delim(path, header = TRUE, stringsAsFactors = FALSE)
  names(ldepth) <- c("CHR", "POS", 
                     paste("MEAN_DEPTH", fname, sep = "_"), paste("VAR_DEPTH", fname, sep = "_"))
  
  # read missing stats
  filename <- paste(fname, ".lmiss", sep = "")
  path <- file.path(dir, filename)
  lmiss <- read.delim(path, header = TRUE, stringsAsFactors = FALSE) %>%
    select(CHR, POS, F_MISS)
  names(lmiss) <- c("CHR", "POS", paste("MISS", fname, sep = "_"))
  # join stats
